...
import {
  LayoutDashboard, ClipboardList, Users, HeartHandshake,
  Building2, Handshake, ShoppingBag, Package, Medal,
  ToggleLeft, MessageSquare, BarChart2, ScrollText,
...
const NAV_GROUPS = [
  {
    label: 'Overview',
    items: [
      { href: '/hq/command', label: 'Command',  icon: LayoutDashboard },
    ],
  },
  {
    label: 'People',
    items: [
      { href: '/hq/registrations', label: 'Registrations', icon: ClipboardList  },
      { href: '/hq/participants',  label: 'Participants',  icon: Users          },
      { href: '/hq/volunteers',    label: 'Volunteers',    icon: HeartHandshake },
    ],
  },
  {
    label: 'Partners & Sponsors',
    items: [
      { href: '/hq/sponsors', label: 'Sponsors', icon: Building2 },
  ...
  },
  {
    ...
    items: [
      ...
      { href: '/hq/inventory',    label: 'Inventory',    icon: Package     },
      { href: '/hq/collectibles', label: 'Collectibles', icon: Medal       },
    ],
  },
  {
    label: 'Operations',
    items: [
      { href: '/hq/phase',          label: 'Phase',          icon: ToggleLeft    },
      { href: '/hq/communications', label: 'Communications', icon: MessageSquare },
      { href: '/hq/reports',        label: 'Reports',        icon: BarChart2     },
...
export function Sidebar() {
  ...
  const panel = (
    <aside className="flex flex-col h-full w-64 bg-navy overflow-y-auto px-3 py-5">
      {/* Wordmark */}
      <div className="flex items-center gap-2.5 px-3 mb-6">
        ...
        <div className="min-w-0">
          ...
          <p className="text-white/40 text-[10px] font-semibold uppercase tracking-wider">HQ Admin</p>
        </div>
        <button
      ...
      <nav className="flex-1 space-y-5">
        {NAV_GROUPS.map(({ label, items }) => (
          <div key={label}>
            <p className="px-3 mb-1 text-[10px] font-black uppercase tracking-widest text-white/25">
              {label}
            </p>
      ...
      <div className="border-t border-white/10 pt-4 mt-4">
        {user?.email && (
          <p className="px-3 mb-2 text-white/40 text-xs truncate">{user.email}</p>
        )}
        <button
          onClick={handleSignOut}
          disabled={exiting}
          className="flex items-center gap-3 w-full px-3 py-2 rounded-xl text-sm font-semibold
